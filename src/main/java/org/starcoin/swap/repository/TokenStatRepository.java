...
public interface TokenStatRepository extends JpaRepository<TokenStat, String> {
    ...
    @Query(value = "select token_name,now() as ts, sum(tvl) as tvl, sum(tvl_amount) as tvl_amount,sum(volume) as volume, sum(volume_amount) as volume_amount  from token_swap_day_stat group by token_name", nativeQuery = true)
    List<TokenStat> sum();

    @Query(value = "select token_name,now() as ts, sum(tvl) as tvl, sum(tvl_amount) as tvl_amount,sum(volume) as volume, sum(volume_amount) as volume_amount  from token_swap_day_stat where token_name=:token_name group by token_name", nativeQuery = true)
